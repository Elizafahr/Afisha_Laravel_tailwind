        ...
        <div class="md:w-2/3 space-y-8">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <!-- @if($activeBookings->isEmpty())
                    <p class="text-gray-500">Нет активных бронирований</p>
                @else
                    <div class="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-600 mt-1">
                                            ...
                                            <div class="flex items-center mt-1">
                                                ...
                                                {{ $booking->event->start_datetime->format('d.m.Y H:i') }}
                                            </div>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-medium">{{ number_format($booking->total_price, 2) }} ₽</div>
                                        <span class="inline-block px-2 py-1 text-xs rounded-full {{ $booking->status === 'confirmed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                            {{ $booking->status === 'confirmed' ? 'Подтверждено' : 'Ожидание' }}
                                        </span>
                                    </div>
                                </div>
                                <a href="{{ route('bookings.show', $booking) }}" class="inline-flex items-center text-sm text-blue-600 hover:text-blue-800 mt-2">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <!-- @if($pastBookings->isEmpty())
                    <p class="text-gray-500">Нет прошедших бронирований</p>
                @else
                    <div class="space-y-4">
                            ...
                            <div class="border-b pb-4 last:border-0">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <h3 class="font-medium text-lg">{{ $booking->event->title }}</h3>
                                        <div class="text-sm text-gray-600 mt-1">
                                            Было {{ $booking->event->start_datetime->format('d.m.Y') }}
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-medium">{{ number_format($booking->total_price, 2) }} ₽</div>
                                 ...
                                 @if($booking->status === 'confirmed' && !$user->reviews->contains('event_id', $booking->event->id))
                                    <a href="{{ route('reviews.create', $booking->event) }}" class="inline-flex items-center text-sm text-green-600 hover:text-green-800 mt-2">
                                        Написать отзыв
                                        <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
