        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="md:w-1/3">
                <div class="bg-white rounded-lg shadow-md p-6 sticky top-4">
                    ...
                    <!-- Статистика -->
                    <div class="grid grid-cols-2 gap-2 text-center border-t pt-4">
                        <div>
                        ...
                        @if (auth()->id() == $user->id)
                            <div class="mt-6">
                                <a href="{{ route('profile.edit') }}"
                                    class="block w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium py-2 px-4 rounded transition">
            ...
            <div class="md:w-2/3 space-y-8">
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    @if ($activeBookings->isEmpty())
                        <p class="text-gray-500">У вас нет активных бронирований</p>
                    @else
                        <div class="space-y-4">
                            @foreach ($activeBookings as $booking)
                                <div class="border rounded-lg p-4 hover:shadow-md transition-shadow">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <h3 class="font-medium text-lg">{{ $booking->event->title }}</h3>
                                            <div class="flex items-center mt-1 text-sm text-gray-600">
                                                ...
                                                <span>{{ $booking->event->start_datetime->format('d.m.Y H:i') }}</span>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-3 grid md:grid-cols-3 gap-4">
                                        <div>
                                            <p class="text-sm text-gray-500">Количество билетов</p>
                                        ...
                                        <div>
                                            <p class="text-sm text-gray-500">Номер бронирования</p>
                                            <p class="font-mono font-medium">{{ $booking->booking_id }}</p>
                                        </div>
                                    </div>

                                    @if ($booking->seats->isNotEmpty())
                                        <div class="mt-3">
                                            <p class="text-sm text-gray-500">Места:</p>
                                            <div class="flex flex-wrap gap-2 mt-1">
                                                @foreach ($booking->seats as $seat)
                                                    <span
                                                        class="bg-blue-100 text-blue-800 text-xs font-medium px-2 py-0.5 rounded">
                                                        Ряд {{ $seat->seat_row }}, Место {{ $seat->seat_number }}
                                                    </span>
                                                @endforeach
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-4 flex justify-end space-x-2">
                                        <a href="{{ route('bookings.show', $booking) }}"
                                            class="px-3 py-1 text-sm bg-blue-100 hover:bg-blue-200 text-blue-800 rounded transition">
                                            Подробнее
                                        </a>
                                        <form action="{{ route('bookings.cancel', $booking) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="px-3 py-1 text-sm bg-red-100 hover:bg-red-200 text-red-800 rounded transition">
                                                Отменить
                                            </button>
                                        </form>
                                    </div>
