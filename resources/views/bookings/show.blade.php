...
<p><strong>Название:</strong> {{ $booking->event?->title ?? 'Мероприятие недоступно' }}</p>
                        <p><strong>Дата:</strong> {{ $booking->event?->start_datetime?->format('d.m.Y H:i') ?? 'Не указана' }}</p>
<p><strong>Место:</strong> {{ $booking->event?->location ?? 'Не указано' }}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <p><strong>Место:</strong>
                            {{ $booking->seat->zone }}, ряд {{ $booking->seat->seat_row }}, место {{ $booking->seat->seat_number }}
                        </p>
                        @endif
